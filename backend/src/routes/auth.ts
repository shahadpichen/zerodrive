...
router.get('/callback/google', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Encode Google tokens for frontend (one-time use in redirect)
    // Frontend will immediately encrypt and store in sessionStorage
    const tokenData = {
      accessToken,
      refreshToken,
    ...
    // Redirect to frontend with encoded tokens (one-time use)
    res.redirect(`${FRONTEND_URL}/oauth/callback?tokens=${encodedTokens}&new=${isNewUser}&limited=${hasLimitedScope}`);
  } catch (error) {
