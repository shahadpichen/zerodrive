...
describe("Error Handler Middleware", () => {
  ...
  describe("errorHandler middleware", () => {
    it("does not copy request secrets or identity data into logs", () => {
      const req = mockRequest({
        url: "/api/shared-files?exchange=secret-code",
        method: "POST",
        body: {
          recipient_email: "recipient@example.com",
          management_capability_hash: "secret-capability",
        },
        query: { exchange: "secret-code" },
        headers: { cookie: "zerodrive_token=secret-token" },
        ip: "192.0.2.1",
      });
      const res = mockResponse();

      errorHandler(
        ApiErrors.BadRequest("Invalid share"),
        req as Request,
        res as Response,
        mockNext(),
      );
      ...
      expect(
        JSON.stringify((logger.warn as jest.Mock).mock.calls),
      ).not.toContain("recipient@example.com");
      expect(
        JSON.stringify((logger.warn as jest.Mock).mock.calls),
